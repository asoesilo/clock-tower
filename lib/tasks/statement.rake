  ...
  task daily_create: :environment do
    ...
    if statement_period.blank?
      if (date.end_of_month == date)
        statement_period = StatementPeriod.where(to: 'End of Month').first
      elsif (date.beginning_of_month == date)
        statement_period = StatementPeriod.where(to: 'Start of Month').first
      end
    end
